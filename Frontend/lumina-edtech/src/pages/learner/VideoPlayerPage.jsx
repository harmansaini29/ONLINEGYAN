...
import { Play, Pause, ChevronLeft, ChevronDown, CheckCircle, Lock, MoreVertical } from "lucide-react";
import { useParams, useNavigate } from 'react-router-dom'; // Added Params
...
export default function VideoPlayerPage() {
  ...
  // Load Course Data based on URL ID
  useEffect(() => {
    const course = COURSES.find(c => c.id === parseInt(courseId));
    if (course) {
      setCurrentCourse(course);
    } else {
      navigate('/learner/marketplace'); // Redirect if invalid ID
    }
  }, [courseId, navigate]);
    ...
    <div className="h-screen bg-[#0B0C15] flex flex-col text-white overflow-hidden font-sans">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 bg-black relative flex flex-col items-center justify-center group overflow-hidden">
          ...
          {/* Dynamic Thumbnail as placeholder */}
          <img 
        ...
        {/* Sidebar Curriculum */}
        <aside className="w-96 bg-[#0B0C15] border-l border-white/5 flex flex-col z-10 overflow-y-auto">
          <div className="p-4 border-b border-white/5 font-semibold text-white">Course Content</div>
